...
class Garden(db.Model):
    ...
    date_created = db.Column(
        db.DateTime, default=datetime.datetime.now(pytz.timezone("Asia/Kolkata"))
    )
...
def add_record():
    ...
    if request.method == "POST":
        ...
        if (
            len(scientific_name) == 0
            or len(common_name) == 0
            or len(flowering_time) == 0
            or len(identifying_characters) == 0
            or len(economic_importance) == 0
        ):
        ...
        if record2 is None:
            ...
            new_garden = Garden(
                ...
                identifying_characters=identifying_characters,
...
def show_importer():
    ...
    for index, x in ImportedData.iterrows():
        record2 = Garden.query.filter_by(
            scientific_name=str(x["Scientific Name"])
        ).first()
        if record2 is None:
            new_uid = str(uuid.uuid4())
            qr = qrcode.QRCode(version=1, box_size=10, border=5)
            ...
            new_garden = Garden(
                ...
                qr_code=image_data,
                scientific_name=str(x["Scientific Name"]),
                common_name=str(x["Common Name"]),
                flowering_time=str(x["Flowering Time"]),
                identifying_characters=str(x["Identifying Characters"]),
                economic_importance=str(x["Economic Importance "]),
            )
            db.session.add(new_garden)
            db.session.commit()
        else:
